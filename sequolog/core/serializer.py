...
def calendar_fields_to_dict(calendar: CalendarFields | None) -> dict[str, Any] | None:
...
def timeline_to_dict(timeline: Timeline) -> dict[str, Any]:
    ...
    return {
        ...
        "events": [event_to_dict(event) for event in timeline.get_all_events()],
    }
...
def project_to_dict(project: Project) -> dict[str, Any]:
    ...
    return {
        ...
        "timelines": [timeline_to_dict(timeline) for timeline in project.get_all_timelines()],
        "sources": [source_to_dict(source) for source in project.get_all_sources()],
    }
...
def calendar_fields_from_dict(data: dict[str, Any] | None) -> CalendarFields | None:
...
def timeline_from_dict(data: dict[str, Any]) -> Timeline:
    ...
    for event_data in data.get("events", []):
        timeline.add_event(event_from_dict(event_data))
    return timeline
...
def project_from_dict(data: dict[str, Any]) -> Project:
    ...
    for timeline_data in data.get("timelines", []):
        project.add_timeline(timeline_from_dict(timeline_data))
    for source_data in data.get("sources", []):
        project.add_source(source_from_dict(source_data))
    return project
